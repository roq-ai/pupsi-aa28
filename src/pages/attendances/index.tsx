...
function AttendanceListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<AttendanceInterface[]>(
    ...
    () =>
      getAttendances({
        relations: ['user_attendance_student_idTouser', 'user_attendance_teacher_idTouser'],
      }),
  );
              ...
              <Thead>
                <Tr>
                  ...
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_attendance_student_idTouser</Th>
                  )}
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_attendance_teacher_idTouser</Th>
                  )}
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_attendance_student_idTouser?.id}`}>
                          {record.user_attendance_student_idTouser?.email}
                        </Link>
                      </Td>
                    )}
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_attendance_teacher_idTouser?.id}`}>
                          {record.user_attendance_teacher_idTouser?.email}
                        </Link>
                      </Td>
                    )}
